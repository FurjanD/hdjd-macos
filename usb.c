...
void
usb_debug_msg(char *action, int ep, const uint8_t data[], size_t datalen)
{
#ifdef DEBUG
...
usb_setup(char *name, size_t namelen)
{
  ...
  if (libusb_pollfds_handle_timeouts(context) == 0) {
    warn("libusb requires timeout handling outside the poll file descriptors.");
  }
  //----------------------------------------------------------------------------
  // Get device list
  //----------------------------------------------------------------------------
...
void
usb_write(const uint8_t *data, size_t datalen)
{
